...
public class Character : MonoBehaviour
{
    ...
    [SerializeField] float extraBounceOffset = 0.1f;
    ...
    [Header("SoundFX")]
    [SerializeField] LayerMask grassGroundLayers;
    [SerializeField] LayerMask sandGroundLayers;
    [SerializeField] LayerMask dirtGroundLayers;
    [SerializeField] LayerMask waterGroundLayers;
    [SerializeField] AudioSource feetAudioSource;
    [SerializeField] AudioClip[] jumpSounds;
    [SerializeField] AudioClip[] sandFootstepsSounds;
    [SerializeField] AudioClip[] grassFootstepsSounds;
    [SerializeField] AudioClip[] dirtFootstepsSounds;
    [SerializeField] AudioClip[] waterFootstepsSounds;
    ...
    public bool layDownRequested;
    public bool standUpRequested;
    public int footstepLayer;
    ...
    void WallJump()
    {
        if (wallJumpRequested)
        {
            if (!isFrozen)
            {
                ...
                //move a little bit not to stuck to the wall
                rb.MovePosition(new Vector2(transform.position.x + (-velocity * extraBounceOffset), transform.position.y));
                rb.velocity = newVelocity;
    ...
    void SetAnimation()
    {
        animator.SetBool(Constants.AnimationParameters.Param_IsFrozen, isFrozen);
        if (!isFrozen)
        {
            animator.SetFloat(Constants.AnimationParameters.Param_Velocity, velocity);
            animator.SetBool(Constants.AnimationParameters.Param_IsMoving, velocity != 0);
            animator.SetBool(Constants.AnimationParameters.Param_IsGrounded, isGrounded);
            if (firstJumpRequested || secondJumpRequested)
                animator.SetTrigger(Constants.AnimationParameters.Trigger_Jump);
        }
        if (layDownRequested)
        {
            animator.SetTrigger(Constants.AnimationParameters.Trigger_LayDown);
            layDownRequested = false;
        }
        if (standUpRequested)
        {
            animator.SetTrigger(Constants.AnimationParameters.Trigger_StandUp);
    ...
    public void Idle()
    {
        animator.Play(Constants.AnimationParameters.State_Character_Idle);
    }

    void PlayRandomJumpSound()
    {
        Utils.PlayRandomSound(audioSource, jumpSounds);
    }
    void PlayRandomWallSound()
    {
        Utils.PlayRandomSound(feetAudioSource, dirtFootstepsSounds);
    }

    void PlayFootstepSound()
    {
        var layerIndex = GetGroundLayer();

        if (grassGroundLayers == (grassGroundLayers | (1 << layerIndex)))
        {
            Utils.PlayRandomSound(feetAudioSource, grassFootstepsSounds);
            Debug.Log("grass footstep");
        }
        else if (sandGroundLayers == (sandGroundLayers | (1 << layerIndex)))
        {
            Utils.PlayRandomSound(feetAudioSource, sandFootstepsSounds);
            Debug.Log("sand footstep");
        }
        else if (dirtGroundLayers == (dirtGroundLayers | (1 << layerIndex)))
        {
            Utils.PlayRandomSound(feetAudioSource, dirtFootstepsSounds);
            Debug.Log("dirt footstep");
        }
        else if (waterGroundLayers == (waterGroundLayers | (1 << layerIndex)))
        {
            Utils.PlayRandomSound(feetAudioSource, waterFootstepsSounds);
            Debug.Log("water footstep");
        }
        else
            Debug.Log("unknown footstep");
